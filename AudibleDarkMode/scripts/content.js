...
chrome.storage.local.get(["AudibleDark"]).then((result) => {
  const dark = result.AudibleDark
  console.log("My Log: ", dark)
  if (dark) {
...
function setAudibleDark(value) {
  if (value !== true || value !== false)
    return "Invalid Input"

  chrome.storage.local.set({ AudibleDark: val });
  return "Success"
}
